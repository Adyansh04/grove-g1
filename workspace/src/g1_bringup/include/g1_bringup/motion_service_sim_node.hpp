 ...
 * rt/lowstate); nothing in the sim services rt/arm_sdk, and with nothing commanding the legs
 * the sim robot collapses. This node closes that gap kinematically: it subscribes /arm_sdk
 * (what g1_hardware_interface's G1ArmSdkSystem publishes) and /lowstate (what unitree_mujoco
 * publishes), and is the ONLY thing in this stack allowed to publish /lowcmd, and only when
 * launched by sim.launch.py.
 *
