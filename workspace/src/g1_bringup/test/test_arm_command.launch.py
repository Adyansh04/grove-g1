...
Ordered acquire, blend weight ramp 0->1, hold-in-place tracking, slew-clamped
FollowJointTrajectory convergence, deactivate ramp-down, reactivate, and
rogue-publisher advisory guard. Run via `colcon test --packages-select g1_bringup`.
"""
...
# Displacement for the slew-clamp trajectory step. Implies ~2 rad/s to
# comfortably exceed the 1.0 rad/s clamp.
...
# Best-effort, matching how unitree_mujoco publishes them.
...
# Pelvis-pin bounds to separate standing from collapsing.
PELVIS_SPAWN_Z = 0.793
PELVIS_MIN_Z = 0.70
PELVIS_MAX_TILT_DEG = 15.0
PELVIS_MAX_XY_M = 0.15

# Deeper history to ensure the terminal weight-0 publish is captured.
...
def generate_test_description():
    sim_launch = IncludeLaunchDescription(
        ...
        # Explicitly pin pelvis to keep deterministic.
...
class TestArmCommand(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Separate node to prevent sim callbacks from starving the test loop.
    ...
    def _run_ros2_run(self, executable, timeout_s=30.0):
        """Runs executable while continuing to spin this node."""
    ...
    def test_full_arm_command_sequence(self):
        # 1. /arm_sdk carries no traffic before activation.
        weight_samples = []
        ...
        # 2. Trigger activation and capture the first tick.
        ...
        # 3. Weight ramps monotonically 0 -> 1 without snapping.
        ...
        # 4. Commanded arm position tracks measured /joint_states while holding.
        hold_sample = {}
        ...
        # 5. Step-like FollowJointTrajectory saturates the slew clamp.
        client = ActionClient(
        ...
        self.assertIsNotNone(
            converged_at,
            "trajectory did not converge within the slew-limited timescale",
        )

        # Bounds: Clamp respected, and velocity saturated.
        self.assertLessEqual(
        ...
        self.assertGreater(
            ...
            "observed joint velocity never approached the slew clamp",
        )
        self.assertGreater(converged_at, goal_sent_at)

        # 6. Clean-stop coverage: deactivate_arm ramps weight to ~0.
        deactivate_samples = []
        ...
        self.assertLessEqual(
            ...
            "weight took longer than blend_ramp_down_s to reach 0.0",
        )
        ...
        # 7. Reactivate for the rogue-publisher guard test.
        self._run_ros2_run("activate_arm")
        self._spin_for(BLEND_RAMP_UP_S + 2.0)

        # 8. Rogue-publisher guard: a second /arm_sdk publisher must trigger ramp-down.
        guard_samples = []
        ...
        self.assertLess(
            min(weights_only),
            0.9,
            "component did not ramp down after rogue /arm_sdk publisher",
        )

        # Isolate component-originated ramp-down values.
        settling_samples = [(t, w) for t, w in guard_samples if w < 1.0 - 1e-6]
        self.assertGreater(
            len(settling_samples),
            0,
            "no component-originated samples observed during/after the ramp",
        )
        self.assertAlmostEqual(
            settling_samples[-1][1],
            0.0,
            delta=1e-2,
            msg="component weight did not settle near 0 after guard tripped",
        )
        ...
        self.assertEqual(
            ...
            "component kept publishing after rogue guard should have quieted",
        )

    def test_pelvis_stayed_pinned_through_sequence(self):
        # Verify the pelvis pin held throughout the test sequence.
