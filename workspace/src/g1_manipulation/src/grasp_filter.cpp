...
applyGripperOffset(const geometry_msgs::msg::Pose& generated, const std::array<double, 6>& xyz_rpy)
{
    ...
    rotation.setRPY(xyz_rpy[3], xyz_rpy[4], xyz_rpy[5]);
    const tf2::Transform offset(rotation, tf2::Vector3(xyz_rpy[0], xyz_rpy[1], xyz_rpy[2]));
...
std::array<double, 3> approachAxis(const geometry_msgs::msg::Pose& generated)
{
    ...
    // Third column: the pose's own +z, expressed in the outer frame.
