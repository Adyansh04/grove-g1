...
/// Activates the arm component and controller, then each hand. Idempotent: the arm is very
/// often already acquired, so finding it so costs nothing.
...
/// Hands the arm and hands back. Always SUCCESS: a release that reported failure would fail the
/// tree it is cleaning up after, and the executor releases again on its way out regardless.
