...
stack while either arm alone succeeds. See `docs/notes`.

## The arm bracket belongs to the executor

`docs/CONTROL_MODES.md` rule 4 asks that a skill release control authority cleanly on success *or*
failure. At mission scope the only place that can be guaranteed is around the whole tree, so the
...
| `test_authority_drift` | The acquire sequence against `g1_bringup`'s `activate_arm`, and that the arm comes first with the hands behind it. |
