    ...
    : rclcpp::Node("g1_mock_detector", options)
{
    ...
    timer_ = create_wall_timer(std::chrono::duration<double>(1.0 / std::max(rate_hz, 0.1)),
                               [this]() { publishMasks(); });
}
    ...
    g1_msgs::msg::InstanceMask&          out) const
{
    ...
    const tf2::Vector3 centre{ detection.bbox.center.position.x, detection.bbox.center.position.y,
...
void G1MockDetector::publishMasks()
{
    ...
    for (const vision_msgs::msg::Detection3D& detection : truth_->detections)
    {
        ...
        const auto         match = std::find_if(phrases_.begin(), phrases_.end(),
                                                [&class_id](const std::string& phrase) {
